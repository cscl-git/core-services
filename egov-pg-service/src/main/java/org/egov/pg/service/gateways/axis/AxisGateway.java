...
public class AxisGateway implements Gateway {
	...
	public AxisGateway(RestTemplate restTemplate, Environment environment, ObjectMapper objectMapper) {
		...
		// VPC_ACCESS_CODE =
		// environment.getRequiredProperty("axis.merchant.access.code");
		// VPC_VERSION = environment.getRequiredProperty("axis.merchant.vpc.version");
		// VPC_COMMAND_PAY =
		// environment.getRequiredProperty("axis.merchant.vpc.command.pay");
		// VPC_COMMAND_STATUS =
		// environment.getRequiredProperty("axis.merchant.vpc.command.status");
		// MERCHANT_URL_PAY = environment.getRequiredProperty("axis.url.debit");
		// MERCHANT_URL_STATUS = environment.getRequiredProperty("axis.url.status");
	}
	...
	// fields.put("vpc_OrderInfo", (String)
	// transaction.getAdditionalFields().get(BANK_ACCOUNT_NUMBER));
	// fields.put("vpc_Amount",
	// String.valueOf(Utils.formatAmtAsPaise(transaction.getTxnAmount())));
	//
	...
	// UriComponentsBuilder.fromHttpUrl(MERCHANT_URL_PAY).queryParams
	// (params).build().encode();
	//
	...
	// @Override
	// public Transaction fetchStatus(Transaction currentStatus, Map<String, String>
	// params) {
	...
	// Map<String, String> fields = new HashMap<>();
	//
	// String txnRef = StringUtils.isEmpty(currentStatus.getModule()) ?
	// currentStatus.getTxnId()
	...
	//
	// UriComponents uriComponents =
	// UriComponentsBuilder.fromHttpUrl(MERCHANT_URL_STATUS).queryParams(queryParams)
	// .build().encode();
	//
	// try {
	// ResponseEntity<String> response =
	// restTemplate.postForEntity(uriComponents.toUriString(), "", String.class);
	//
	// log.info(response.getBody());
	//
	// Map<String, List<String>> responseParams =
	// AxisUtils.splitQuery(response.getBody());
	//
	// log.info(responseParams.toString());
	//
	// return transformRawResponse(responseParams, currentStatus);
	// } catch (RestClientException e) {
	// log.error("Unable to fetch status from payment gateway for txnid: " +
	// currentStatus.getTxnId(), e);
	// throw new ServiceCallException("Error occurred while fetching status from
	// payment gateway");
	// }
	// }

	// private Transaction transformRawResponse(Map<String, List<String>> resp,
	...
	// break;
	// case "D":
	// respMsg = "Deferred transaction has been received and is awaiting
	// processing";
	// break;
	// case "E":
	...
	// break;
	// case "L":
	// respMsg = "Shopping Transaction Locked (Please try the transaction again
	// later)";
	// break;
	// case "M":
	...
	// break;
	// case "P":
	// respMsg = "Transaction has been received by the Payment Adaptor and is being
	// processed";
	// break;
	// case "R":
	// respMsg = "Transaction was not processed - Reached limit of retry attempts
	// allowed";
	// break;
	// case "S":
