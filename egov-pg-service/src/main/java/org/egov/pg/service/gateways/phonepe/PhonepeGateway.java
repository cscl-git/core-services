...
public class PhonepeGateway implements Gateway {
	...
	public URI generateRedirectURI(Transaction transaction) {
		...
		try {
			...
			String encodedPayload = new String(
					Base64.getEncoder().encode(objectMapper.writeValueAsString(map).getBytes()));
			...
			String uri = UriComponentsBuilder.newInstance().scheme("https").host(MERCHANT_HOST)
					.path(MERCHANT_PATH_DEBIT).build().toUriString();

			HttpEntity<Map<String, String>> httpEntity = new HttpEntity<>(
					Collections.singletonMap("request", encodedPayload), httpHeaders);
	...
	public Transaction fetchStatus(Transaction currentStatus, Map<String, String> params) {
		...
		path.append(MERCHANT_PATH_STATUS).append("/").append(MERCHANT_ID).append("/").append(currentStatus.getTxnId())
				.append("/status");
		...
		try {
			...
			ResponseEntity<PhonepeResponse> response = restTemplate.exchange(uri, HttpMethod.GET, httpEntity,
	...
	private Transaction transformRawResponse(PhonepeResponse resp, Transaction currentStatus) {
		...
		if (resp.getSuccess()) {
			...
			return Transaction.builder().txnId(currentStatus.getTxnId())
					.txnAmount(Utils.convertPaiseToRupee(resp.getAmount())).txnStatus(status)
					.gatewayTxnId(resp.getProviderReferenceId()).gatewayStatusCode(resp.getCode())
					.gatewayStatusMsg(resp.getMessage()).responseJson(resp).build();
		} else {
			if (resp.getCode().equalsIgnoreCase("PAYMENT_PENDING"))
				status = Transaction.TxnStatusEnum.PENDING;
			else
				status = Transaction.TxnStatusEnum.FAILURE;
			return Transaction.builder().txnId(currentStatus.getTxnId()).txnStatus(status)
					.gatewayTxnId(resp.getProviderReferenceId()).gatewayStatusCode(resp.getCode())
					.gatewayStatusMsg(resp.getMessage()).responseJson(resp).build();
		}
