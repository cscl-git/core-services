...
public class PaytmGateway implements Gateway {
	...
	private Transaction transformRawResponse(PaytmResponse resp, Transaction currentStatus) {
		...
		return Transaction.builder().txnId(currentStatus.getTxnId())
				.txnAmount(Utils.formatAmtAsRupee(
						resp.getTxnAmount().equals("") || resp.getTxnAmount() == null ? "0" : resp.getTxnAmount()))
				.txnStatus(status).gatewayTxnId(resp.getTxnId()).gatewayPaymentMode(resp.getPaymentMode())
				.gatewayStatusCode(resp.getRespCode()).gatewayStatusMsg(resp.getRespMsg()).responseJson(resp).build();
