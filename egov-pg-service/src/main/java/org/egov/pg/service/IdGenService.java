...
public class IdGenService {
	...
	@Autowired
	public IdGenService(IdGenRepository idGenRepository, TransactionRepository transactionRepository,
	 ...
	 *            Request for which ID should be generated
	...
	String generateRefundTxnId(RefundTransactionRequest transactionRequest) {
		RefundTransaction txn = transactionRequest.getRefundTransaction();
		IdGenerationResponse response = idGenRepository.getId(transactionRequest.getRequestInfo(), txn.getTenantId(),
				appProperties.getIdGenName(), appProperties.getIdGenRefundFormat(), 1);
		String txnId = response.getIdResponses().get(0).getId();
		log.info("Transaction ID Generated: " + txnId);
		return txnId;
