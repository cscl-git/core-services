...
atom.responseIV_Salt=8E41C78439831010F81F61C344B7BFC7
	...
	public URI generateRedirectURI(Transaction transaction) {
		...
		String signature = AtomTechUtils.getEncodedValueWithSha2(REQ_HASH_KEY, MERCHANT_ID, ATOM_PASSWORD, TRANSACTION_TYPE, PRODUCT_ID, transactionId, amount, CURRENCY);
		
			URI uri = UriComponentsBuilder.fromHttpUrl(GATEWAY_URL)
					.queryParam("login", MERCHANT_ID)
					.queryParam("pass", ATOM_PASSWORD)
					.queryParam("ttype", TRANSACTION_TYPE)
					.queryParam("prodid", PRODUCT_ID)
					.queryParam("amt", amount)
					.queryParam("txncurr", CURRENCY)
					...
					.queryParam("ru", transaction.getCallbackUrl())
	...
	public Transaction fetchStatus(Transaction currentStatus, Map<String, String> params) {
		...
		URI uri = UriComponentsBuilder.fromHttpUrl(TXN_STATUS_URL)
				.queryParam("merchantid", MERCHANT_ID)
				.queryParam("merchanttxnid", transactionId)
				.queryParam("amt", currentStatus.getTxnAmount())
				.queryParam("tdate", new SimpleDateFormat("yyyy-MM-dd").format(new Date(createdTime))).build(false).toUri();
		ResponseEntity<String> response = this.restTemplate.getForEntity(uri, String.class);
		try {
			...
			Transaction newTransaction = Transaction.builder()
			        ...
			        .gatewayStatusMsg(statusResponse.getGatewayTransactionStatus())
			        .responseJson("")
			        .build();
